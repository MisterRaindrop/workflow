...
## 13. 每个容器各存一份镜像 —— 已解决，附完整实测

**问题最初被描述成「下载太慢，能不能把 Docker 共享起来」。测下来不是下载。**
镜像早就在 host 上，`wk warm` 走的是 `docker save | docker load`，全程不碰网络。
真正的成本在别处：
...
| host 侧 tar | 13.3 GB |
| 容器内展开 | 27 GB |
| `docker load` 一次 | **373 秒** |
| 同一镜像存了几份 | **3 份**（tar 13G + 两个容器各 27G ≈ 67G） |
| 造一个容器总耗时 | apt 约 20 分钟 + load 6.2 分钟 ≈ **26 分钟** |

而且每造一次都要重赌一次 apt。

**为什么不能共享 `/var/lib/docker`**：两个 daemon 指向同一个目录会互相破坏
元数据。这条路是技术禁忌，不是取舍问题。

**做法：CoW 存储池 + 克隆黄金容器**。这是 LXD 方案本来就有、而 wk 一直没用上的
优势。`lxc copy` 在 btrfs/zfs 池上是写时复制：块共享，各容器保留自己的可写视图。

**逐项实测（169 上，均已清理测试痕迹）**：

| 项目 | dir/xfs（现状） | btrfs CoW |
|---|---|---|
| 拿到可用环境 | ≈ 26 分钟 | **49 秒**（`lxc copy` < 1s + 启动 + 等 dockerd） |
| 额外磁盘 | 27 GB | **0**（池占用 27.69GiB → 27.69GiB） |
| 完整编译一次 | 23 分 08 秒 | **22 分 55 秒**（失败点相同，工作量可比） |
| 顺序写 2G | 176 MB/s | 237 MB/s |
| 8000 个小文件 | 1135 ms | 563 ms |
| 三个容器各持 27GB 镜像 | 逻辑 81GB+ | 池实际 **27.69GiB** |

IO 那两行要打折看：loop 层有写缓冲，容器内 `fsync` 未必落到物理盘。但编译这
一项是端到端计时，**22:55 对 23:08，性能无损失**这个结论是硬的。

**安全性验证**（这是与共享 daemon 目录的本质区别）：在一个克隆体里 `docker rmi`
掉共享镜像，另一个的**仍然存在且能跑**；各自新增的镜像互不可见。

**克隆时必须丢掉两样东西**，否则会留下难查的问题：

- **源容器绑定的代码目录**。继承下来意味着两个容器写同一份 checkout。
- **`/etc/machine-id`**。`lxc copy` 原样复制，于是两台「机器」共用一个身份，
  journald 和任何以它为键的东西都会当真。清空文件（而非删除）是 systemd
  下次启动时重新生成的信号，而克隆出来的容器此刻还是 stopped，正好赶在首次启动前。

**还踩到一个只在克隆路径上出现的坑**：克隆体的 dockerd 是从零启动的，容器到
RUNNING 时它还没在监听（实测约 20 秒）。从零造的路径天然不会遇到 —— 因为
`install_docker` 最后一步就是 `docker info`。少了这个等待，verify 会把一个
好容器报成不可用。

**代价**（都是真实的，写在这里免得日后重新发现）：

- 池只能建在文件上（这台机器没有空闲块设备），**fstab 那行不能少**，否则重启后池就没了。
- 黄金容器要在它携带的镜像换版本时重做一次。
- 现有 dir 池里的容器不会自动受益；要迁过去得重建。

**顺带的发现**：这台机器的 xfs 开了 `reflink=1`，`cp -a --reflink=always` 复制
一份 11G 的代码树**只要 12 秒且零额外占用**。也就是说「准备一份新任务的代码」
同样不必每次 `git clone` 12G 再跑一遍 submodule 与 maven 缓存播种。
